...
function loadData(string $dataFile): array
{
    if (!file_exists($dataFile)) {
        $default = ['nextId' => 1, 'users' => []];
        saveData($dataFile, $default);
        return $default;
    }
    ...
    if (!is_array($data) || !isset($data['nextId']) || !isset($data['users']) || !is_array($data['users'])) {
        throw new \RuntimeException('Invalid data file structure');
    }
